...
export const openFileTool = tool({
    description:
        "Read a file and display its contents to the user. Returns a summary with file metadata. The full content is shown to the user automatically. If you need the actual file content for analysis, use the read_file_content tool instead.",
    inputSchema: z.object({
        path: z
            .string()
            .describe("The path to the file relative to the root directory"),
    }),
    execute: async (input) => {
        ...
        try {
            ...
            const { summary, lineCount, byteSize, fileType, preview } =
                generateContentSummary(content, input.path)
...
export const createFileTool = tool({
    ...
    inputSchema: z.object({
        path: z
            .string()
            .describe(
                "The path for the new file relative to the root directory",
            ),
        content: z
            .string()
    ...
    execute: async (input) => {
        const allowed = await checkPermission("create_file", {
            path: input.path,
...
export const renameFileTool = tool({
    description:
        "Rename a file. Can also be used to move a file to a different location.",
    inputSchema: z.object({
        oldPath: z
            .string()
            .describe(
                "The current path to the file relative to the root directory",
            ),
        newPath: z
            .string()
            .describe(
                "The new path for the file relative to the root directory",
            ),
    }),
    execute: async (input) => {
        const allowed = await checkPermission("rename_file", {
...
export const moveFileTool = tool({
    ...
    inputSchema: z.object({
        ...
        destinationPath: z
            .string()
            .describe(
                "The destination path for the file relative to the root directory",
            ),
    }),
    execute: async (input) => {
        ...
        try {
            await fileSystemManager.renameFile(
                input.sourcePath,
...
export const deleteFileTool = tool({
    ...
    inputSchema: z.object({
        path: z
            .string()
            .describe(
                "The path to the file to delete relative to the root directory",
            ),
    }),
    execute: async (input) => {
        const allowed = await checkPermission("delete_file", {
            path: input.path,
        })
        if (!allowed) {
...
export const getFileMetadataTool = tool({
    ...
    execute: async (input) => {
        const allowed = await checkPermission("get_file_metadata", {
            path: input.path,
        })
        if (!allowed) {
...
export const catTool = tool({
    description:
        "Display the contents of a file (like Unix cat command). Shows full content to the user but returns a summary. If you need the actual content for analysis, use read_file_content instead.",
    inputSchema: z.object({
        path: z
            .string()
            .describe("The path to the file relative to the root directory"),
...
export const grepTool = tool({
    description:
        "Search for a pattern in files (like Unix grep command). Searches in specified file or all files if no path provided. Returns matching lines with line numbers.",
    inputSchema: z.object({
        ...
        path: z
            ...
            .optional()
            .describe(
                "Optional: specific file path to search in. If not provided, searches all files",
            ),
        caseInsensitive: z
            .boolean()
            .optional()
    ...
    execute: async (input) => {
        const allowed = await checkPermission("grep", {
...
export const headFileTool = tool({
    description:
        "Read the first N lines of a file. Similar to the Linux head command. Use this to preview the beginning of a file without reading the entire content. Maximum 10,000 lines.",
    inputSchema: z.object({
        path: z
            .string()
            .describe("The path to the file relative to the root directory"),
        lines: z
            ...
            .default(10)
            .describe(
                "Number of lines to read from the beginning (default: 10, max: 10,000)",
            ),
    }),
    execute: async (input) => {
        const allowed = await checkPermission("head_file", {
            path: input.path,
            lines: input.lines,
        })
        if (!allowed) {
...
export const tailFileTool = tool({
    description:
        "Read the last N lines of a file. Similar to the Linux tail command. Use this to view the end of a file, such as recent log entries or the end of a document. Maximum 10,000 lines.",
    inputSchema: z.object({
        path: z
            .string()
            .describe("The path to the file relative to the root directory"),
        lines: z
            ...
            .default(10)
            .describe(
                "Number of lines to read from the end (default: 10, max: 10,000)",
            ),
    }),
    execute: async (input) => {
        const allowed = await checkPermission("tail_file", {
            path: input.path,
            lines: input.lines,
        })
        if (!allowed) {
...
export const cpTool = tool({
    ...
    inputSchema: z.object({
        source: z
            .string()
            .describe(
                "The path to the source file relative to the root directory",
            ),
        destination: z
            .string()
            .describe(
                "The destination path for the copy relative to the root directory",
            ),
    }),
    execute: async (input) => {
...
export const mkdirTool = tool({
    ...
    inputSchema: z.object({
        path: z
            .string()
            .describe(
                "The path for the new directory relative to the root directory",
            ),
    }),
    execute: async (input) => {
...
export const treeTool = tool({
    ...
    inputSchema: z.object({
        path: z
            ...
            .optional()
            .describe(
                "Optional: starting directory path. If not provided, shows the entire tree",
            ),
        maxDepth: z
            .number()
            .int()
            ...
            .optional()
            .describe(
                "Optional: maximum depth to display (0 = root only, default: unlimited, max: 20)",
            ),
    }),
    execute: async (input) => {
        ...
        try {
            ...
            for (const entry of filteredEntries) {
                ...
                const relativePath = basePath
                    ? entry.path.slice(basePath.length + 1) || entry.name
                    : entry.path
...
export const diffTool = tool({
    ...
    execute: async (input) => {
        const allowed = await checkPermission("diff", {
            file1: input.file1,
            file2: input.file2,
        })
        if (!allowed) {
        ...
        if (isChange) {
            ...
            const findLineNumbers = (
                ...
                for (
                    let i = startIdx;
                    i < changes.length &&
                    (foundLine1 === undefined || foundLine2 === undefined);
                    i++
                ) {
                    const c = changes[i]!
            ...
            if (hunkStart === -1) {
                ...
                for (
                    ...
                    c <= Math.min(lastChangeIdx + contextLines, idx - 1);
                    c++
                ) {
                    const ctx = changes[c]!
                    if (ctx.type === "equal" && ctx.line1 !== undefined) {
...
export const wcTool = tool({
    ...
    inputSchema: z.object({
        path: z
            ...
            .optional()
            .describe(
                "Optional: file path. If not provided, counts for all files",
            ),
        countLines: z
            .boolean()
            .optional()
            .default(true)
            .describe("Count lines (default: true)"),
        countWords: z
            .boolean()
            .optional()
            .default(true)
            .describe("Count words (default: true)"),
        countChars: z
            .boolean()
            .optional()
            .default(true)
            .describe("Count characters (default: true)"),
    }),
    execute: async (input) => {
        ...
        try {
            ...
            for (const filePath of filesToCount) {
                try {
                    ...
                    const lines = input.countLines
                        ? (content.match(/\n/g) || []).length
                        : 0
                    const words = input.countWords
...
export const sortTool = tool({
    ...
    inputSchema: z.object({
        ...
        reverse: z
            ...
            .optional()
            .default(false)
            .describe("Sort in reverse order (default: false)"),
        numeric: z
            .boolean()
            .optional()
            .default(false)
        ...
        unique: z
            ...
            .optional()
            .default(false)
            .describe("Remove duplicate lines (default: false)"),
        ignoreCase: z
            .boolean()
            .optional()
            .default(false)
...
export const uniqTool = tool({
    ...
    inputSchema: z.object({
        ...
        count: z
            ...
            .default(false)
            .describe(
                "Prefix lines with count of occurrences (default: false)",
            ),
        duplicatesOnly: z
            .boolean()
            .optional()
    ...
    execute: async (input) => {
        ...
        try {
            ...
            if (input.count) {
                outputLines = filtered.map(
                    (item) =>
                        `${item.count.toString().padStart(7)} ${item.line}`,
                )
            } else {
...
export const readFileContentTool = tool({
    ...
    inputSchema: z.object({
        ...
        maxLines: z
            ...
            .default(500)
            .describe(
                "Maximum number of lines to return (default: 500, max: 1000)",
            ),
        startLine: z
            .number()
            .int()
    ...
    execute: async (input) => {
        const allowed = await checkPermission("read_file_content", {
            path: input.path,
        })
        if (!allowed) {
...
/**
 * Interface for file system operations needed by pipes
 */
export interface PipeableFileSystem {
...
const PIPEABLE_TOOL_PERMISSIONS: Record<PipeableToolName, ToolName> = {
    ...
    echo: "open_file", // echo uses open_file permission for simplicity (reading/writing text)
}
...
const pipeableFunctions: Record<
    PipeableToolName,
    (
        fs: PipeableFileSystem,
        args: Record<string, unknown>,
        stdin?: string,
    ) => Promise<PipeableResult>
> = {
    /**
     * Cat - read file(s) or pass through stdin
     */
    cat: async (fs, args, stdin) => {
        ...
        if (!paths && !path) {
            ...
            return {
                success: false,
                error: "cat: no input (provide paths or pipe input)",
            }
        }
        ...
        for (const filePath of filePaths) {
            try {
                ...
                return {
                    success: false,
                    error: `cat: ${filePath}: ${(error as Error).message}`,
                }
            }
        }
    ...
    read_file: async (fs, args) => {
        ...
        try {
            ...
            return {
                success: false,
                error: `read_file: ${(error as Error).message}`,
            }
        }
    },
    ...
    grep: async (fs, args, stdin) => {
        ...
        if (path) {
            try {
                ...
                return {
                    success: false,
                    error: `grep: ${path}: ${(error as Error).message}`,
                }
            }
            ...
            return {
                success: false,
                error: "grep: no input (provide path or pipe input)",
            }
        }
    ...
    sort: async (fs, args, stdin) => {
        ...
        if (path) {
            try {
                ...
                return {
                    success: false,
                    error: `sort: ${path}: ${(error as Error).message}`,
                }
            }
        } else if (stdin !== undefined) {
            content = stdin
            ...
            return {
                success: false,
                error: "sort: no input (provide path or pipe input)",
            }
        }
    ...
    uniq: async (fs, args, stdin) => {
        ...
        if (path) {
            try {
                ...
                return {
                    success: false,
                    error: `uniq: ${path}: ${(error as Error).message}`,
                }
            }
        } else if (stdin !== undefined) {
            content = stdin
            ...
            return {
                success: false,
                error: "uniq: no input (provide path or pipe input)",
            }
        }
        ...
        if (count) {
            outputLines = filtered.map(
                (item) => `${item.count.toString().padStart(7)} ${item.line}`,
            )
        } else {
            outputLines = filtered.map((item) => item.line)
        }
    ...
    head: async (fs, args, stdin) => {
        ...
        if (path) {
            try {
                ...
                return {
                    success: false,
                    error: `head: ${path}: ${(error as Error).message}`,
                }
            }
        } else if (stdin !== undefined) {
            content = stdin
            ...
            return {
                success: false,
                error: "head: no input (provide path or pipe input)",
            }
        }
    ...
    tail: async (fs, args, stdin) => {
        ...
        if (path) {
            try {
                ...
                return {
                    success: false,
                    error: `tail: ${path}: ${(error as Error).message}`,
                }
            }
        } else if (stdin !== undefined) {
            content = stdin
            ...
            return {
                success: false,
                error: "tail: no input (provide path or pipe input)",
            }
        }
    ...
    wc: async (fs, args, stdin) => {
        ...
        if (path) {
            try {
                ...
                return {
                    success: false,
                    error: `wc: ${path}: ${(error as Error).message}`,
                }
            }
        } else if (stdin !== undefined) {
            content = stdin
            ...
            return {
                success: false,
                error: "wc: no input (provide path or pipe input)",
            }
        }
     ...
     */
    write_file: async (fs, args, stdin) => {
        const path = args.path as string
        ...
        if (content === undefined) {
            return {
                success: false,
                error: "write_file: no content (provide content or pipe input)",
            }
        }
        ...
        try {
            ...
            return {
                success: false,
                error: `write_file: ${(error as Error).message}`,
            }
        }
    },

    /**
     * Echo - output text or pass through stdin
     */
    echo: async (fs, args, stdin) => {
        const text = args.text as string | undefined
        let content = text !== undefined ? text : stdin
        if (content) {
...
- echo: Output text or pass through input. Args: { text?: string }
...
{ commands: [{ tool: "read_file", args: { path: "src/main.ts" } }, { tool: "grep", args: { pattern: "^import" } }, { tool: "sort", args: {} }] }`,
    inputSchema: z.object({
        commands: z
            .array(pipeCommandSchema)
            .min(1)
            .describe(
                "Commands to execute in sequence. Output of each becomes input to the next.",
            ),
        debug: z
            .boolean()
            .optional()
            .default(false)
            .describe(
                "If true, include intermediate results in output for debugging",
            ),
    }),
    execute: async (input) => {
        ...
        const pipeAllowed = await checkPermission("pipe", {
            commands: commands.map((c) => c.tool),
        })
        if (!pipeAllowed) {
        ...
        for (const cmd of commands) {
            const permissionName =
                PIPEABLE_TOOL_PERMISSIONS[cmd.tool as PipeableToolName]
            if (!permissionName) {
...
/**
 * Execute a chain of pipeable commands (for terminal use)
 * This is a simplified version of pipeTool that doesn't require permissions
 * and is designed to be called directly from the terminal
 */
export async function executePipeableChain(
    fs: PipeableFileSystem,
    ...
    for (let i = 0; i < commands.length; i++) {
        ...
        if (!pipeableFn) {
            return { success: false, error: `Unknown tool: ${cmd.tool}` }
        }

        const result = await pipeableFn(fs, cmd.args || {}, currentOutput)
